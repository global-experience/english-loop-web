...
import { SmartLocationSettings } from "./SmartLocationSettings";
...
export function SettingsView({
  ...
  const [internalLoading, setInternalLoading] = useState<boolean>(!initialUser);
  ...
  useEffect(() => {
    if (!initialUser && !user) {
      ...
      Promise.resolve(apiFetch<User>("/api/me"))
        .then((data) => {
  ...
  useEffect(() => {
    ...
    setLearningPresetState(readLearningPresets());
  ...
  useEffect(() => {
    const handlePull = (e: Event) => {
      ...
      if (customEvent.detail?.tab === "settings") {
        void onSaved().finally(() => {
  ...
  if (!user || internalLoading || externalLoading) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const repeats = [0, 1, 2].map((index) => Number(form.get(`repeat_${index}`))) as [number, number, number];
    const speeds = [0, 1, 2].map((index) => Number(form.get(`speed_${index}`))) as [number, number, number];
    if (new Set(repeats).size !== 3 || new Set(speeds).size !== 3) {
      setMessage("반복 횟수와 배속은 각각 서로 다른 값 3개를 지정해 주세요.");
      return;
    }
    saveLearningPresets({ repeats, speeds });
    setLearningPresetState({ repeats, speeds });
  ...
  return <div className="view-stack settings-content-enter"><header className="view-title"><p className="eyebrow">SETTINGS</p><h2>계정과 데이터를<br />가볍게 정리하기.</h2></header>
    ...
    <SmartLocationSettings variant="full" />
