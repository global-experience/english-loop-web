...
import { DatabaseBackup, Download, HardDrive, KeyRound, LogOut, Save, ShieldCheck } from "lucide-react";
...
export function SettingsView({ user, onSaved }: { user: User; onSaved: () => Promise<void> }) {
  ...
  useEffect(() => { void navigator.storage?.estimate().then((estimate) => setUsage(`${formatBytes(estimate.usage || 0)} / ${formatBytes(estimate.quota || 0)}`)); }, []);
