...
export function ReportView({ active = true }: { active?: boolean }) {
  ...
  return <div className="view-stack"><header className="view-title report-title"><div><p className="eyebrow">PROGRESS, NOT POINTS</p><h2>점수보다 중요한<br/>실제 사용의 변화.</h2></div><div className="segmented"><button className={days === 7 ? "active" : ""} onClick={() => setDays(7)}>7일</button><button className={days === 14 ? "active" : ""} onClick={() => setDays(14)}>14일</button></div></header>
    {analytics && <><section className="metric-grid"><Metric Icon={MessageCircle} label="자발적 사용률" value={`${analytics.target_expression_usage.spontaneous_rate}%`} detail={`${analytics.target_expression_usage.spontaneous}/${analytics.target_expression_usage.tracked} 표현`}/><Metric Icon={Timer} label="총 학습 시간" value={`${analytics.total_study_minutes}`} suffix="분" detail={`${days}일 누적`}/><Metric Icon={Award} label="새로 MASTERED" value={`${analytics.newly_mastered}`} suffix="개" detail="반복 성공"/><Metric Icon={ChartNoAxesColumnIncreasing} label="발화 일치도" value={analytics.speech.average_match == null ? "—" : `${analytics.speech.average_match}`} suffix={analytics.speech.average_match == null ? undefined : "점"} detail={analytics.speech.average_improvement == null ? `따라 말하기 ${analytics.speech.attempts}회` : `${analytics.speech.average_improvement > 0 ? "+" : ""}${analytics.speech.average_improvement}점 · ${analytics.speech.attempts}회`}/><Metric Icon={MessageCircle} label="연습한 자막" value={`${practicedLines}`} suffix="개" detail={`다시 말할 문장 ${retryLines}개`}/></section>
      {profile?.data_quality && <section className="latest-report"><div className="section-heading"><div><p className="eyebrow">AI PERSONALIZATION</p><h2>내 학습 정보 연결 상태</h2></div><strong>{profile.data_quality.confidence}</strong></div><p>근거 {profile.data_quality.evidence_count}건 · 음성 {profile.data_quality.speech_attempts_14d}회 · 피드 {profile.data_quality.feed_events_14d}건 · 코치 리포트 {profile.data_quality.coach_reports}개</p><div className="compact-list"><article><span className="status-dot"/><div><strong>ChatGPT {profile.integration?.latest_coach_session?.status || "NOT_STARTED"}</strong><small>컨텍스트 {profile.integration?.latest_coach_session?.context_version || profile.profile_version} · 분석 근거 {profile.integration?.latest_coach_session?.report_evidence_count || 0}건</small></div></article>{profile.recommended_actions?.map((item) => <article key={`${item.type}-${item.title}`}><span className="status-dot warning"/><div><strong>{item.title}</strong><small>{item.reason}</small></div></article>)}{profile.content_preferences?.top_channels?.slice(0, 3).map((item) => <article key={item.channel}><span className="status-dot"/><div><strong>{item.channel}</strong><small>학습 열기 {item.opens}회 · 저장 {item.saves}회</small></div></article>)}</div>{(profile.integration?.unfinished_sessions ?? 0) > 0 && <p className="muted-copy">저장되지 않은 ChatGPT 세션이 {profile.integration?.unfinished_sessions}개 있어요.</p>}</section>}
      <section className="routine-chart"><div className="section-heading"><div><p className="eyebrow">ROUTINE CONSISTENCY</p><h2>루틴 완료율</h2></div></div>{Object.entries(analytics.routine).map(([slot, stat]) => <div className="bar-row" key={slot}><span>{slotLabel(slot)}</span><div><i style={{ width: `${stat.completion_rate}%` }}/></div><strong>{stat.completion_rate}%</strong></div>)}</section>
